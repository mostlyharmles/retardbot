...
class VideoCommands(commands.Cog):
    ...
    async def gifitize(self, ctx):
        try:
            ...
            if not video_attachment:
                await ctx.send(
                    "No video found in recent messages. Please attach a video, reply to a message with a video, or ensure there's a video in recent chat history."
                )
                return

            if not video_attachment.filename.lower().endswith(
                (".mp4", ".mov", ".avi", ".webm")
            ):
